  ...
  for (let i = 0; i < tokens.length; i++) {
    ...
    if (i + 2 < tokens.length) {
      ...
      if (/^\s+$/.test(ws) && c0.toLowerCase() === "art" && c2.toLowerCase().startsWith("installation")) {
        ...
        if (p2) result.push(<span key={`p-ai-${i}`}>{p2}</span>);
        i += 2;
        continue;
      }
    }
    ...
    if (core === "Argentina") {
      ...
      if (punct) result.push(<span key={`p-arg-${i}`}>{punct}</span>);
      continue;
    }
    ...
    if (core === "Berlin") {
      ...
      if (punct) result.push(<span key={`p-ber-${i}`}>{punct}</span>);
      continue;
    }
    ...
    if (isAnim) {
      ...
      if (punct) result.push(<span key={`p-${i}`}>{punct}</span>);
      continue;
    }
