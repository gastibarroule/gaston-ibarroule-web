...
function DownloadButtons() {
  ...
  const macBtn = (
    <a href="/downloads/Sonidata_Embed_1.6.0_universal.dmg" className={os === "win" ? SECONDARY : PRIMARY}>
...
const WHATS_NEW_FEATURES = [
  {
    emoji: "⚡️",
    title: "Direct LAN Beam",
    description:
      "Beam complete sessions wirelessly from Sonidata iOS directly into Embed on your Mac — no cables or cloud needed.",
  },
  {
    emoji: "📁",
    title: "Library Organizer",
    description:
      "Auto-detect UCS categories and organize your studio WAV library without exporting CSVs.",
  },
  {
    emoji: "🌓",
    title: "Monochrome Design",
    description:
      "Refined dark monochrome UI designed to match the Sonidata iOS experience.",
  },
];
...
function WhatsNew() {
  ...
  return (
    <div className="mt-8 max-w-2xl mx-auto">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex items-center justify-center gap-2 text-sm font-medium text-neutral-400 hover:text-white transition-colors py-3 group"
      >
        <span className="inline-block w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" />
        <span>What&apos;s new in v1.6.0</span>
        <svg
          width="14"
          height="14"
          viewBox="0 0 24 24"
          ...
          className={`transform transition-transform duration-300 ${isOpen ? "rotate-180" : ""} text-neutral-600 group-hover:text-neutral-400`}
        >
      ...
      >
        <div className="grid gap-3 pt-2 pb-4">
          {WHATS_NEW_FEATURES.map((f) => (
            <div
              key={f.title}
              className="flex items-start gap-3 bg-white/[0.03] border border-white/8 rounded-xl px-4 py-3.5 transition-colors hover:bg-white/[0.05]"
            >
      ...
      <section id="download" className="w-full max-w-4xl mx-auto px-4 mb-28 scroll-mt-24">
        <div className="bg-neutral-900/50 border border-white/10 rounded-[32px] p-8 md:p-12 relative overflow-hidden">
          ...
          <div className="text-center mb-10">
            ...
            <span className="inline-block text-xs font-medium text-neutral-500 border border-white/10 rounded-full px-3 py-1">v1.6.0</span>
          </div>
          ...
          <p className="text-center text-sm text-neutral-500 max-w-lg mx-auto mt-6">
